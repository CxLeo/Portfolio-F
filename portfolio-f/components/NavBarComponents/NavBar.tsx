...
const NavBar = () => {
  ...
  useEffect(() => {
    ...
    if (themeLocal==='light') {
      setSrcImg('/images/navbar/sun.png');
      setThemeState(themeLocal);
    }else if (themeLocal==='dark'){
      setSrcImg('/images/navbar/moon.png');
      setThemeState(themeLocal);
    }
  });
  ...
  const navbar = {
    open: {
      ...
      backgroundColor: cn(theme === 'system' ? 'black' : theme === 'dark' ? 'black' : 'white'),
  ...
  //   switch (localStorage.getItem('themes')) {
  //     case 'light':
    ...
    >
      <div className="w-full flex-grow">
        <div className="w-full flex ">
          ...
          <Image alt="asd" src={srcImg} width={19} height={19} className="  w-[19px] h-[19px]  mt-[26px]  lg:ml-[80.7%] ml-[60%] cursor-pointer hover:-translate-y-[2px] transition-transform duration-200 transform "
            onClick={() => {
